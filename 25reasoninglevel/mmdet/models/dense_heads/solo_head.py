...
class SOLOHead(nn.Module):
    ...
    def forward_single(self, x, idx,  img_metas=None, human_feats=None, eval=False, upsampled_size=None):
        ins_kernel_feat = x
        ...
        # cate branch
        human_feats = F.interpolate(human_feats, size=seg_num_grid, mode='bilinear', align_corners=True).contiguous()
        cate_feat = cate_feat.contiguous()
        for i, cate_layer in enumerate(self.cate_convs):
            if i ==self.insert_point:
                cate_feat=self.reasoning_list[idx](cate_feat, human_feats=human_feats)
