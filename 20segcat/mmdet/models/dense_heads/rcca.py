...
class CC_module(nn.Module):
    ...
    def forward(self, x,human):
        ...
        proj_query = human
        proj_query = self.query_conv(proj_query)
        ...
        '''
        proj_key_H = proj_query_H.permute(0,2,1)
        proj_key_W = proj_query_W.permute(0,2,1)
        # n*c*h*w->n*h*c*w->nh*c*w
        proj_value=x
        proj_value = self.value_conv(proj_value)
...
class RCCA(nn.Module):
    ...
    def forward(self, feats,human, eval=False):
        for i in range(self.recurrence):
            feats=self.cc_module(feats,human)
        return feats
