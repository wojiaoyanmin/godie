...
class SOLOHead(nn.Module):
    ...
    def forward(self, feats,img_metas=None, eval=False):
        ...
        plt.show()
        '''
        edge_feats,edge_pred = self.cate_feat_head(feats)
        ins_pred = self.mask_feat_head(feats[:-1],edge_feats)
        ...
        return cate_pred, kernel_pred, edge_pred,ins_pred
    ...
    def loss(self,
        ...
        loss_edge =self.loss_edge(edge_pred.sigmoid(),human_edge_label)
    ...
    def human_edge_single(self,
        ...
        output_stride=4
        device = gt_instance_raw[0].device
        human_label = torch.zeros( mask_feat_size[0]*output_stride, mask_feat_size[1]*output_stride, dtype=torch.int64, device=device)
        human_ids = torch.unique(gt_instance_raw)
        for ids in human_ids:
            ...
            gt_masks = gt_masks_raw[keep.cpu().numpy()]
            gt_masks = gt_masks.to_ndarray()
            gt_masks = np.sum(gt_masks, axis=0).astype(np.bool)
            gt_masks_expand=np.zeros((mask_feat_size[0]*output_stride, mask_feat_size[1]*output_stride),dtype=np.bool)
            gt_masks_expand[:gt_masks.shape[0],:gt_masks.shape[1]]=gt_masks
            #gt_masks_tensor = torch.Tensor(gt_masks)
            human_label[gt_masks_expand]=ids+1
        human_label = human_label.cpu().numpy()
        human_label=cv2.resize(human_label,(mask_feat_size[1],mask_feat_size[0]),interpolation=cv2.INTER_NEAREST)
        human_label=self.generate_edge(human_label)
        human_label=torch.from_numpy(human_label).to(device).unsqueeze(0).long()

        return human_label,None
