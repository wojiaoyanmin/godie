...
class SOLOHead(nn.Module):
    ...
    def forward(self, feats,img_metas=None, eval=False):
        ...
        for i in range(self.num_classes):
            ...
            plt.subplot(2,3,5)
            plt.imshow(img)
            size=(cate_pred[0][0][0].shape[-2]*4,cate_pred[0][0][0].shape[-1]*4)
            plt.subplot(2,3,1)
            plt.imshow(cate_pred[0][0][i].detach().cpu().numpy())
            plt.subplot(2,3,2)
            plt.imshow(cate_pred[1][0][i].detach().cpu().numpy())
            plt.subplot(2,3,3)
            plt.imshow(cate_pred[2][0][i].detach().cpu().numpy())
            plt.subplot(2,3,4)
            plt.imshow(cate_pred[3][0][i].detach().cpu().numpy())
            plt.show()
        '''
